/*
 * Created by JFormDesigner on Sat Jan 02 00:33:58 CST 2021
 */
 ...
 * @author unknown
 */
public class WorkerU extends JFrame {
    public static String IDnumber;
    public WorkerU() {
        initComponents();
    }
    public WorkerU(String zhanghao) {
        IDnumber = zhanghao;
    }
    ...
    private void button3ActionPerformed(ActionEvent e) throws SQLException {
        if(comboBox2.getSelectedItem().toString().equals("移除员工信息")){
            boolean rn = DBManager.getINSTANCE().executeUpdate("delete from worker where  WorkerID = \""+textField3.getText()+"\"");
            if(rn){
            ...
            boolean rn = DBManager.getINSTANCE().executeUpdate("update worker set " + comboBox2.getSelectedItem().toString() + " = \"" + textField4.getText() + "\" where WorkerID = \"" + textField3.getText() + "\"");
            if (rn) {
                Dialog.setVisible(true);
            } else {
                Dialog5.setVisible(true);
    ...
    private void button9ActionPerformed(ActionEvent e) throws SQLException {
        Dialog5.dispose();
        String sql = "update Worker set "+comboBox2.getSelectedItem().toString()+" = \""+textField4.getText()+"\" where WorkerID = \""+textField3.getText()+"\"";
    ...
    private void initTable(JTable table1,String WorkerID,String type,String object) throws SQLException {
        DefaultTableModel model = (DefaultTableModel) table1.getModel();
        Object[][] arr;
        if(type.equals("移除员工信息")){
            String[] list = {"移除员工信息","员工ID", "员工姓名"};
            ...
            arr = new Object[1][3];
            ResultSet rs =  DBManager.getINSTANCE().executeQuery("select * from worker where WorkerID = \""+WorkerID+"\"");
            if (rs.next()) {
            ...
            String[] list = {"员工ID", "员工姓名","修改属性","原信息","修改信息"};
    ...
    private void button1ActionPerformed(ActionEvent e) throws SQLException {
        ...
        if(rs.next()) {
            if(rs.getString("WorkerType").equals("管理员") && !rs.getString("WorkerID").equals(IDnumber))
            {
                Dialog6.setVisible(true);
            }
            else{
                if(comboBox2.getSelectedItem().toString().equals("WorkerPhone")){
    ...
    private void initComponents() {
        ...
        //======== Dialog6 ========
        {
            ...
            //---- button11 ----
            button11.setText("\u8fd4\u56de\u4fe1\u606f\u4fee\u6539\u754c\u9762");
            button11.addActionListener(e -> button6ActionPerformed(e));
            Dialog6ContentPane.add(button11, BorderLayout.PAGE_END);

            //---- label11 ----
            label11.setText("\u60a8\u6ca1\u6709\u4fee\u6539\u6b64ID\u4fe1\u606f\u7684\u6743\u9650\uff01");
            label11.setFont(label11.getFont().deriveFont(label11.getFont().getSize() + 1f));
            label11.setHorizontalAlignment(SwingConstants.CENTER);
            Dialog6ContentPane.add(label11, BorderLayout.CENTER);
            Dialog6.setSize(200, 200);
